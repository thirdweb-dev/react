 ...
 * @beta
 */
export function useEditionDropList(
  contract: Erc1155<any> | undefined,
  queryParams?: QueryAllParams,
) {
  const contractAddress = contract?.getAddress();
  return useQueryWithNetwork(
    cacheKeys.contract.queryAllEditionDrop(contractAddress, queryParams),
    async () => {
 ...
 * @beta
 */
export function useEditionDropSupply(
  contract: Erc1155<any> | undefined,
  tokenId: BigNumberish,
) {
  const contractAddress = contract?.getAddress();
  return useQueryWithNetwork(
    cacheKeys.contract.totalSupplyEditionDrop(contractAddress, tokenId),
    async () => {
      if (contract) {
    ...
    {
      enabled: !!tokenId && (!!contract || !contractAddress),
    },
  );
}
